...
mediante el script \texttt{semana\_3/scripts/carga\_ms.py}.
Cada archivo fue descomprimido en memoria, parseado línea a línea como
JSON~Lines (\texttt{.geojsonl}), y cada polígono fue validado con Shapely
(corrección triple: \texttt{make\_valid} + \texttt{buffer(0)} + orientación
\textit{counter-clockwise}).
Únicamente se retuvieron las edificaciones cuyo centroide cae dentro del
área de unión (\texttt{unary\_union}) de los 170~municipios PDET.
El resultado de este primer paso —\textbf{filtrado espacial}— se guardó
en el archivo intermedio
\texttt{semana\_3/datos/procesados/microsoft\_pdet\_filtrado.csv}
(\textbf{5\,259~filas}, \textbf{1.3~MB}), con las geometrías aún en
formato WKT.

En un segundo paso, el script \texttt{limpieza.py} convirtió cada
geometría de WKT a GeoJSON serializado como cadena Python
(compatible con PyMongo~4.x), forzó la orientación CCW
(\textit{counter-clockwise}) requerida por el índice
\texttt{2dsphere}, y descartó la única fila que contenía la coordenada
problemática \texttt{-74.77944668605295}.
El resultado final fue \textbf{5\,258~registros} en el archivo
\texttt{semana\_3/datos/procesados/microsoft\_pdet\_limpio.csv}
(\textbf{1.6~MB}).
...
El Listado~\ref{lst:ms_ejemplo_fila} muestra un registro representativo
del archivo \texttt{microsoft\_pdet\_limpio.csv} tal como fue leído desde
disco antes de la inserción en MongoDB.

\begin{lstlisting}[caption={Ejemplo de fila real en \texttt{microsoft\_pdet\_limpio.csv}},
                   label={lst:ms_ejemplo_fila}]
geometry: {"type": "Polygon", "coordinates": [[
             [-75.39831400369859, -0.00758691291268],
             [-75.39830998262347, -0.00766615859629],
             [-75.39823507364210, -0.00766235757365],
             [-75.39823909445187, -0.00758311711940],
             [-75.39831400369859, -0.00758691291268]
           ]]}
area_m2 : 73.267
height  : -1.0
...
\noindent
El campo \texttt{geometry} es un diccionario GeoJSON serializado como
cadena de texto; PyMongo lo parsea en tiempo de inserción con
\texttt{json.loads()} antes de almacenarlo como subdocumento.
El valor \texttt{height = -1.0} indica que la altura no está disponible
en ninguno de los 5\,258~registros del subconjunto PDET de la versión
de febrero de 2026.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{lXX}
    ...
    Tamaño colección en MongoDB
      & 0.8~MB total (0.6~MB datos + 0.2~MB índices);
        $\overline{\text{doc}}$~= 342~B
      & 346.6~MB total (256.2~MB datos + 90.4~MB índices);
        $\overline{\text{doc}}$~= 348~B \\[4pt]
    Año de imágenes
      & 2014–2021
